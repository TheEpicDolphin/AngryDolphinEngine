...
TEST(set_trie_test_suite, trie_structure_test)
{
    ...
    test_set_trie.InsertValueForKeySet(node_1, { 3, 5, 6 });
    test_set_trie.InsertValueForKeySet(node_2, { 3, 4, 7 });
    test_set_trie.InsertValueForKeySet(node_3, { 1, 3 });
    test_set_trie.InsertValueForKeySet(node_4, { 4, 5, 6, 7 });
    test_set_trie.InsertValueForKeySet(node_5, { 2, 4, 6, 10 });
    test_set_trie.InsertValueForKeySet(node_6, { 2 });
    test_set_trie.InsertValueForKeySet(node_7, { 1, 2 });
...
TEST(set_trie_test_suite, removal_test)
{
    ...
    SetTrie<uint64_t, std::string> test_set_trie;
    test_set_trie.InsertValueForKeySet(node_1, { 3, 4, 6 });
    test_set_trie.InsertValueForKeySet(node_2, { 3, 4, 7 });
    test_set_trie.InsertValueForKeySet(node_3, { 1, 3 });
    test_set_trie.InsertValueForKeySet(node_4, { 4, 5, 6, 7 });
...
TEST(set_trie_test_suite, value_modification_test)
{
    ...
    std::string& node_val_2 = *test_set_trie.ValueForKeySet({ 3, 4, 7 });
    node_val_2[1] = 'a';
    ASSERT_EQ(*test_set_trie.ValueForKeySet({ 3, 4, 7 }), "nade 2");

    std::string& node_val_3 = *test_set_trie.ValueForKeySet({ 1, 3 });
    node_val_3[2] = 's';
    ASSERT_EQ(*test_set_trie.ValueForKeySet({ 1, 3 }), "nose 3");
}
